...
<div id="container">
    ...
    <div id="body">
	    ...
	    <article>
			...
			$statement = oci_parse($conn,'select * from transactions t,account a where a.cust_id=:username and a.account_id=t.account_id order by t.date_trans desc' );
			oci_bind_by_name($statement,":username",$user_check);
			$r=oci_execute($statement);
			echo "<table border='1'>\n";
			echo "<tr><th>Date</th><th>Amount</th><th>Category</th></tr>";
			while (($row = oci_fetch_row($statement)) != false) 
			{ 
				$amt=abs($row[2]);
				echo " <tr>\n<td>$row[4]</td><td>$amt</td><td>$row[3]</td></tr>\n"; 
				 
			} 
			echo "</table>\n";

			?>
			<br><br><a href="graphs.php" class="button">Click</a> to view graphs and other information.
			</p>
